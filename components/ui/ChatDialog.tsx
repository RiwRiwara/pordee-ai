...
export default function ChatDialog({ isOpen, onClose }: ChatDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 overflow-hidden flex flex-col" style={{ height: '80vh' }}>
        {/* Chat header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[75%] rounded-lg p-3 ${msg.isUser 
                ...
                {msg.isTyping ? (
                  ...
                  <>
                    {msg.isUser ? (
                      <div className="whitespace-pre-line">{formatMessageText(msg.text)}</div>
                    ) : (
                    ...
                    <p className="text-xs mt-1 text-right text-gray-500">{msg.time}</p>
                  </>
                )}
              </div>
