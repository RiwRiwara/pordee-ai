...
export default function TodoMonth() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const monthlyPayments = debts
            .filter((debt: DebtItem) => debt.paymentDueDay && debt.paymentDueDay <= daysInMonth)
            .map((debt: DebtItem) => {
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4 text-[#3776C1]">สิ่งที่ต้องทำในเดือนนี้</h2>
      ...
      {payments.length === 0 ? (
        ...
        <div className="space-y-4">
          {payments.map((payment) => (
            <div key={payment.debtId} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-bold text-lg">ชำระหนี้{payment.debtName} {formatNumber(payment.amount)} บาท</h3>
                    <div className="flex items-center">
                      <span className="text-sm text-gray-500">วันที่ต้องชำระ:</span>
                      ...
                      <div className="mt-2 border-t pt-2">
                        {payment.details.map((detail, index) => (
                          <div key={index} className="flex justify-between items-center py-1">
                            ...
                            <span className="text-sm font-medium text-blue-600">{formatNumber(detail.amount)} บาท</span>
                          </div>
                        ))}
                      </div>
                    )}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <ModalContent>
          ...
          <ModalFooter>
            <Button color="primary" className="w-full" onPress={handleConfirmPayment}>
              ยืนยัน
            </Button>
          </ModalFooter>
        </ModalContent>
