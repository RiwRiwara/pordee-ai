...
export default function TipSection({ debtContext, riskPercentage = 0 }: TipSectionProps) {
  ...
  useEffect(() => {
    if (debtContext && debtContext.debtItems && debtContext.debtItems.length > 0) {
      generateAITip();
    }
  }, [debtContext]);
  ...
  const generateAITip = async () => {
    if (!debtContext || !debtContext.debtItems || debtContext.debtItems.length === 0) return;
    ...
    try {
      ...
      if (riskPercentage > 60) {
        setAiTip("การจัดลำดับความสำคัญของหนี้และการเจรจากับเจ้าหนี้สามารถช่วยลดความเสี่ยงทางการเงินของคุณได้");
      } else if (riskPercentage > 40) {
        setAiTip("การสร้างแผนชำระหนี้ที่ชัดเจนและติดตามค่าใช้จ่ายอย่างใกล้ชิดจะช่วยให้คุณจัดการหนี้ได้ดีขึ้น");
      } else {
        setAiTip("การรักษาวินัยทางการเงินและการออมอย่างสม่ำเสมอจะช่วยให้คุณมีความมั่นคงทางการเงินในระยะยาว");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="p-4">
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 relative overflow-hidden">
        ...
        {riskPercentage > 0 && (
          <div className="absolute top-0 right-0 px-2 py-1 text-xs font-semibold rounded-bl-lg" style={{ backgroundColor: 'rgba(243, 244, 246, 0.8)' }}>
