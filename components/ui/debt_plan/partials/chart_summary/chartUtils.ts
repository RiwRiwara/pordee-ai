...
export const processDebtData = (debtData: DebtItem[]) => {
  ...
  Object.entries(groupedDebts).forEach(([debtType, debts]) => {
    ...
    const groupTotalAmount = debts.reduce(
    ...
    const groupMinMonthlyPayment = debts.reduce((sum, debt) => {
      return (
        sum + (debt.minimumPayment || Math.ceil(debt.remainingAmount * 0.05))
      );
    }, 0);
    ...
    debts.forEach((debt) => {
      ...
      if (interestMethod === InterestCalculationMethod.REDUCING_BALANCE) {
        ...
        for (let month = 1; month <= timeForThisDebt; month++) {
          const monthlyInterest =
            remainingAmount * (debt.interestRate / 100 / 12);
      ...
      if (interestMethod === InterestCalculationMethod.REDUCING_BALANCE) {
        ...
        for (let month = 1; month <= timeForThisDebt; month++) {
          const monthlyInterest =
            remainingAmount * ((debt.interestRate * 0.95) / 100 / 12);

          interestPaid += monthlyInterest;

          const principalPayment = Math.min(
            recommendedPayment - monthlyInterest,
            remainingAmount,
          );
      ...
      debts.forEach((debt) => {
        ...
        if (interestMethod === InterestCalculationMethod.REDUCING_BALANCE) {
          if (month <= originalTimeInMonths) {
            ...
            originalAmountForMonth +=
              debt.remainingAmount *
                Math.pow(1 + debt.interestRate / 100 / 12, month) -
              minPayment *
                ((Math.pow(1 + debt.interestRate / 100 / 12, month) - 1) /
          ...
          if (month <= originalTimeInMonths) {
            originalAmountForMonth += Math.max(
              0,
              debt.remainingAmount * (1 - month / originalTimeInMonths),
            );
          }

          if (month <= newPlanTimeInMonths) {
            newPlanAmountForMonth += Math.max(
              0,
              debt.remainingAmount * (1 - month / newPlanTimeInMonths),
            );
          }
        }
      });
    ...
    result.total.originalTimeInMonths = Math.max(
      result.total.originalTimeInMonths,
      originalTimeInMonths,
    );
    result.total.newPlanTimeInMonths = Math.max(
      result.total.newPlanTimeInMonths,
      newPlanTimeInMonths,
    );
  ...
  result.total.monthlyData = Array.from({ length: maxTotalMonths }, (_, i) => {
    ...
    Object.entries(result).forEach(([type, data]) => {
      ...
      const monthData = data.monthlyData.find(
...
export const generateLineChartData = (debtTypeData: DebtPlanData) => {
    ...
    for (let i = 0; i < maxMonths; i++) {
      ...
      monthlyData.push({
        ...
        originalAmount: Math.max(
          0,
          originalTotalAmount - originalMonthlyRate * month,
        ),
        newPlanAmount: Math.max(
          0,
          newPlanTotalAmount - newPlanMonthlyRate * month,
        ),
      });
    }
  }
  ...
  const completeMonthlyData = Array.from({ length: maxMonths }, (_, i) => {
    ...
    const existingData = monthlyData.find(
      (data: { month: number }) => data.month === month,
    );
    ...
    if (existingData) {
      ...
      return {
        month,
        originalAmount:
          month <= originalTimeInMonths
            ? Math.max(
                0,
                originalTotalAmount * (1 - month / originalTimeInMonths),
              )
            : 0,
        newPlanAmount:
          month <= newPlanTimeInMonths
            ? Math.max(
                0,
                newPlanTotalAmount * (1 - month / newPlanTimeInMonths),
              )
            : 0,
      };
    }
    ...
    completeMonthlyData.push(
      {
        ...
        newPlanAmount: newPlanTotalAmount,
      },
...
export const generateBarChartData = (
  debtDataByType: Record<string, DebtPlanData>,
) => {
