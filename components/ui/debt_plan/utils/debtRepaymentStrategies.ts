...
export type RepaymentStrategy = "Snowball" | "Avalanche" | "PriorityScore" | "Proportional";
 ...
 */
export const sortDebtsByStrategy = (debts: DebtItem[], strategy: RepaymentStrategy): DebtItem[] => {
  ...
  if (strategy === "Snowball") {
    ...
    return debtsCopy.sort((a: DebtItem, b: DebtItem) => a.remainingAmount - b.remainingAmount);
  } 
  else if (strategy === "Avalanche") {
    // Option 2: Highest Interest First (Avalanche)
    // Sort debts by interest rate (highest to lowest)
    return debtsCopy.sort((a: DebtItem, b: DebtItem) => b.interestRate - a.interestRate);
  } 
...
export const applyPaymentWithStrategy = (
  ...
  const totalMinimumPayment = debtsCopy.reduce((sum: number, debt: DebtItem) => {
    const minPayment = debt.minimumPayment || Math.max(debt.remainingAmount * 0.01, 500);
  ...
  let remainingDebts = debtsCopy.filter((debt: DebtItem) => debt.remainingAmount > 0.01);
  ...
  if (extraPayment > 0 && remainingDebts.length > 0) {
    if (strategy === "Proportional") {
      ...
      firstDebt.remainingAmount = Math.max(0, firstDebt.remainingAmount - paymentToApply);
    }
  }
...
export const simulateDebtRepayment = (
  ...
  while (debtsCopy.length > 0 && months < maxMonths) {
    ...
    // Remove fully paid debts
    debtsCopy = debtsCopy.filter((debt: DebtItem) => debt.remainingAmount > 0.01);
  }
