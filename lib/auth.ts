  ...
  try {
    ...
    if (adminToken) {
      try {
        ...
        if (decodedToken && decodedToken.isAdmin && decodedToken.exp > Date.now()) {
          return {
            authenticated: true,
        ...
        console.log('Admin token decode failed:', e);
      }
    }
...
export function withRoleAuth(handler: Function, allowedRoles: UserRole[] = ['admin', 'user']) {
  return async (request: NextRequest, ...args: any[]) => {
    const userInfo = await getUserInfo(request);
