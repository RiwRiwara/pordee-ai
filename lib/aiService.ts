    ...
    async ocr(image: string, options: OCROptions = {}): Promise<string> {
        ...
        try {
            ...
            const imageContent = isUrl
                ...
                : { type: 'image_url', image_url: { url: `data:image/jpeg;base64,${image}`, detail } };
            ...
            const messages: Message[] = [
                systemPrompt,
                {
                    role: 'user',
                    content: [
                        { type: 'text', text: 'Please extract all text from the provided image.' },
                        imageContent,
                    ],
                },
            ];
 ...
 */
export async function updateUserDebtContext(userId: string, context: string): Promise<boolean> {
    try {
...
export function createDebtPrompt(context: DebtContext): string {
    ...
    let highInterestDebts = debtItems
        .filter(debt => parseFloat(debt.interestRate) > 15)
        .map(debt => debt.name);
