...
export const PUT = withAdminAuth(async (request: NextRequest, { params }: { params: { id: string } }) => {
  try {
    ...
    const { email, firstName, lastName, role, isActive, languagePreference } = body;
    ...
    if (role && user.role === 'admin' && role === 'user') {
      ...
      if (adminCount <= 1) {
        return NextResponse.json(
          { error: 'Cannot demote the last admin account. Create another admin first.' },
          { status: 400 }
        );
      }
    }
    ...
    if (isActive === false && user.role === 'admin') {
      ...
      if (activeAdminCount <= 1) {
        return NextResponse.json(
          { error: 'Cannot deactivate the last active admin account. Activate another admin first.' },
          { status: 400 }
        );
      }
    }
