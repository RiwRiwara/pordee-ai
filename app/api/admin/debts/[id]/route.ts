...
export const GET = withAdminAuth(
  async (request: NextRequest, { params }: { params: { id: string } }) => {
    try {
      ...
      // Find debt
      const debt = await Debt.findById(debtId).populate(
        "userId",
  ...
  async (request: NextRequest, { params }: { params: { id: string } }) => {
    try {
      ...
      if (remainingAmount !== undefined)
        debt.remainingAmount = parseFloat(remainingAmount);
      if (interestRate !== undefined)
        debt.interestRate = parseFloat(interestRate);
      if (paymentDueDay !== undefined)
        debt.paymentDueDay = parseInt(paymentDueDay);
      if (minimumPayment !== undefined)
        debt.minimumPayment = parseFloat(minimumPayment);
