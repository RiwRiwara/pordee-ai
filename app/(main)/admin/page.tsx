  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch dashboard statistics using admin authentication cookie
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                <p className="text-sm text-gray-500">
                  จำนวนหนี้เฉลี่ยต่อผู้ใช้
                </p>
                <p className="text-2xl font-bold text-orange-600">
                  {stats.totalUsers > 0
                    ? (stats.totalDebts / stats.totalUsers).toFixed(1)
                    : "0"}
                </p>
              </div>
            </div>
        ...
        <div className="overflow-hidden rounded-lg bg-green-500">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div
            className="cursor-pointer rounded-md bg-blue-50 p-4 transition-colors hover:bg-blue-100"
            role="button"
            tabIndex={0}
            onClick={() => (window.location.href = "/admin/users")}
            onKeyDown={(e) =>
              e.key === "Enter" && (window.location.href = "/admin/users")
            }
          >
            <svg
          ...
          <div
            className="cursor-pointer rounded-md bg-yellow-50 p-4 transition-colors hover:bg-yellow-100"
            role="button"
            tabIndex={0}
            onClick={() => (window.location.href = "/admin/debts")}
            onKeyDown={(e) =>
              e.key === "Enter" && (window.location.href = "/admin/debts")
            }
          >
            <svg
            ...
            <p className="text-center text-sm text-gray-600">
              ดู แก้ไข ลบข้อมูลหนี้
            </p>
          </div>

          <div
            className="cursor-pointer rounded-md bg-green-50 p-4 transition-colors hover:bg-green-100"
            role="button"
            tabIndex={0}
            onClick={() => (window.location.href = "/admin/settings")}
            onKeyDown={(e) =>
              e.key === "Enter" && (window.location.href = "/admin/settings")
            }
          >
            <svg
          ...
          <div
            className="cursor-pointer rounded-md bg-red-50 p-4 transition-colors hover:bg-red-100"
            role="button"
            tabIndex={0}
            onClick={() => (window.location.href = "/admin/logs")}
            onKeyDown={(e) =>
              e.key === "Enter" && (window.location.href = "/admin/logs")
            }
          >
            <svg
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              <tr>
                ...
                <td className="whitespace-nowrap px-6 py-4 text-sm text-green-500">
                  +{stats.recentRegistrations} (7 วัน)
                </td>
              </tr>
              <tr>
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                  ผู้ใช้ที่ยังใช้งานอยู่
                </td>
                <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">
                  {stats.activeUsers}
                </td>
